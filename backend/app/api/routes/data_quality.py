...
async def get_quality_overview(
    ...
    try:
        ...
        overall_score = completeness * 0.3 + consistency * 0.3 + accuracy * 0.2 + timeliness * 0.2
...
async def get_completeness_metrics(
    ...
    total_patients = db.query(func.count(Patient.id)).scalar() or 0
    if total_patients > 0:
        missing_name = db.query(func.count(Patient.id)).filter(Patient.name.is_(None)).scalar() or 0
        missing_gender = (
            db.query(func.count(Patient.id)).filter(Patient.gender.is_(None)).scalar() or 0
        )
        missing_birth_date = (
            db.query(func.count(Patient.id)).filter(Patient.birth_date.is_(None)).scalar() or 0
        )
    ...
    if total_conditions > 0:
        missing_patient_id = (
            db.query(func.count(Condition.id)).filter(Condition.patient_id.is_(None)).scalar() or 0
        )
        missing_code = (
            db.query(func.count(Condition.id))
            .filter(and_(Condition.code.is_(None), Condition.code_text.is_(None)))
            .scalar()
            or 0
        )
        missing_onset = (
            db.query(func.count(Condition.id)).filter(Condition.onset_datetime.is_(None)).scalar()
            or 0
        )
    ...
    if total_encounters > 0:
        missing_patient_id = (
            db.query(func.count(Encounter.id)).filter(Encounter.patient_id.is_(None)).scalar() or 0
        )
        missing_class = (
            db.query(func.count(Encounter.id)).filter(Encounter.encounter_class.is_(None)).scalar()
            or 0
        )
        missing_start_time = (
            db.query(func.count(Encounter.id)).filter(Encounter.period_start.is_(None)).scalar()
            or 0
        )
...
async def get_consistency_metrics(
    ...
    duplicate_patients = (
        db.query(Patient.name, Patient.birth_date, func.count(Patient.id).label("count"))
        .filter(Patient.name.isnot(None), Patient.birth_date.isnot(None))
        .group_by(Patient.name, Patient.birth_date)
...
async def get_accuracy_metrics(
    ...
    invalid_birth_dates = (
        db.query(func.count(Patient.id)).filter(Patient.birth_date > current_date).scalar() or 0
    )
...
async def get_timeliness_metrics(
    ...
    stale_conditions = (
        db.query(func.count(Condition.id))
        .filter(Condition.onset_datetime < stale_threshold)
        .scalar()
        or 0
    )
...
async def get_quality_issues(
    ...
    orphaned_conditions = db.query(Condition).filter(Condition.patient_id.is_(None)).limit(10).all()
    ...
    # High: Invalid dates
    invalid_patients = (
        db.query(Patient).filter(Patient.birth_date > datetime.now().date()).limit(10).all()
    )
    ...
    patients_missing_name = db.query(Patient).filter(Patient.name.is_(None)).limit(10).all()
