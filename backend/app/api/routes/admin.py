    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            response = await client.post(
                f"{ETL_SERVICE_URL}/api/bulk-data/kick-off", json=etl_request
            )
            ...
            if response.status_code == 200:
                ...
                etl_job = ETLJob(
                    ...
                    status="in-progress",
                    fhir_server_url=config.fhirServerUrl,
                    config={"since": config.since, "resource_types": config.resourceTypes},
                ...
                if response.status_code == 200:
                    ...
                    if job:
                        ...
                        if job.start_time:
                            ...
                            time_start = job.start_time - timedelta(
                                hours=6
                            )  # Include 6 hours before
                            time_end = datetime.now() + timedelta(hours=1)  # Include up to now

                            patient_count = (
                                db.query(func.count(Patient.id))
                                .filter(
                                    Patient.created_at >= time_start, Patient.created_at <= time_end
                                )
                                .scalar()
                                or 0
                            )
                            ...
                            logger.info(
                                f"Counted {actual_loaded} actual DB records (P:{patient_count} C:{condition_count} E:{encounter_count} O:{observation_count})"
                            )
                        else:
                            # Fallback: use FHIR server counts
                            if "total_resources" in status_data:
                                job.records_processed = status_data["total_resources"]
                            elif "result" in status_data and isinstance(
                                status_data["result"], dict
                            ):
                                result_data = status_data["result"]
                                if "output" in result_data:
                                    total_records = sum(
                                        f.get("count", 0) for f in result_data["output"]
                                    )
                        ...
                        if job_status == "completed":
                            ...
                            db.commit()
                            logger.info(
                                f"Job {job_id} completed successfully with {job.records_processed} records"
                            )
                            break
                            ...
                            job.error_log = json.dumps(
                                {"error": error_msg, "status_data": status_data}, ensure_ascii=False
                            )
                            db.commit()
    ...
    if attempt >= max_attempts:
        try:
            ...
            if job and job.status == "in-progress":
                ...
                job.end_time = datetime.now()
                job.error_log = json.dumps(
                    {"error": "Monitoring timeout after 10 minutes"}, ensure_ascii=False
                )
                db.commit()
...
async def delete_etl_job(
    ...
    return {"message": "ETL job deleted successfully", "job_id": job_id}


@router.get("/valuesets")
async def get_valuesets(
    current_user: dict = Depends(require_role(["admin", "engineer"])), db: Session = Depends(get_db)
):
    """Get all valuesets"""
...
async def update_valueset(
    ...
    return {"id": valueset.id, "message": "Valueset updated successfully"}
...
async def delete_valueset(
    ...
    return {"message": "Valueset deleted successfully"}
