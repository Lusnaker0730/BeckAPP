...
async def get_audit_logs(
    ...
    try:
        ...
        return {"total": total, "skip": skip, "limit": limit, "logs": logs_data}
...
async def get_audit_stats(
    ...
    try:
        ...
        # 总操作数
        total_operations = (
            db.query(func.count(AuditLog.id)).filter(AuditLog.timestamp >= start_date).scalar()
        )

        # 失败操作数
        failed_operations = (
            db.query(func.count(AuditLog.id))
            .filter(and_(AuditLog.timestamp >= start_date, AuditLog.is_success == "failure"))
            .scalar()
        )
        ...
        operations_by_resource = (
            db.query(AuditLog.resource, func.count(AuditLog.id).label("count"))
            .filter(and_(AuditLog.timestamp >= start_date, AuditLog.resource.isnot(None)))
        ...
        top_users = (
            ...
            .filter(and_(AuditLog.timestamp >= start_date, AuditLog.username.isnot(None)))
        ...
        daily_operations = (
            db.query(
                func.date(AuditLog.timestamp).label("date"), func.count(AuditLog.id).label("count")
            )
            .filter(AuditLog.timestamp >= start_date)
        ...
        return {
            ...
            "summary": {
                ...
                "success_rate": round(
                    (1 - (failed_operations or 0) / (total_operations or 1)) * 100, 2
                ),
                "active_users": active_users or 0,
            },
            "by_action": [
                {"action": action, "count": count} for action, count in operations_by_action
            ],
            "by_resource": [
                {"resource": resource, "count": count} for resource, count in operations_by_resource
            ],
            "top_users": [
                {"username": username, "operations": count} for username, count in top_users
            ],
            "daily_trend": [
                {"date": str(date), "operations": count} for date, count in daily_operations
            ],
        }
