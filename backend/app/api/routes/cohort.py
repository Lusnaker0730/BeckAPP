...
async def create_cohort(
    ...
    try:
        ...
        patient_count = await _calculate_cohort_size(db, cohort_data.criteria)
        ...
        logger.info(
            f"Created cohort: {new_cohort.name} (ID: {new_cohort.id}) with {patient_count} patients"
        )
...
async def get_cohort(
    cohort_id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    获取指定群组详情
    """
    ...
    if not cohort:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Cohort {cohort_id} not found"
        )
...
async def get_cohort_statistics(
    ...
    gender_dist = (
        ...
        .filter(Patient.fhir_id.in_(patient_fhir_ids))
    ...
    patients = (
        db.query(Patient.birth_date)
        .filter(Patient.fhir_id.in_(patient_fhir_ids), Patient.birth_date.isnot(None))
        .all()
    )
    ...
    top_conditions_query = (
        ...
        .filter(Condition.patient_id.in_(patient_fhir_ids), Condition.code_text.isnot(None))
    ...
    top_conditions = [
        {"condition": row.code_text, "count": row.count} for row in top_conditions_query
    ]

    # Encounters
    encounters_count = (
        db.query(func.count(Encounter.id))
        .filter(Encounter.patient_id.in_(patient_fhir_ids))
        .scalar()
        or 0
    )
...
async def get_cohort_patients(
    ...
    return {"total": cohort.patient_count, "skip": skip, "limit": limit, "patients": patient_list}
...
async def compare_cohorts(
    ...
    cohorts = (
        db.query(Cohort)
        .filter(Cohort.id.in_(comparison.cohort_ids), Cohort.is_active == True)
        .all()
    )

    if len(cohorts) != len(comparison.cohort_ids):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="One or more cohorts not found"
        )
...
async def _compare_outcomes(db: Session, cohorts: List[Cohort]) -> Dict[str, Any]:
    ...
    for cohort in cohorts:
        ...
        total_conditions = (
            ...
            .filter(Condition.patient_id.in_(patient_fhir_ids))
            .scalar()
            or 0
        )

        unique_conditions = (
            db.query(func.count(func.distinct(Condition.code_text)))
            .filter(Condition.patient_id.in_(patient_fhir_ids), Condition.code_text.isnot(None))
            .scalar()
            or 0
        )
...
async def _compare_trends(db: Session, cohorts: List[Cohort]) -> Dict[str, Any]:
    ...
    for cohort in cohorts:
        ...
        yearly_data = (
            ...
            .filter(Encounter.patient_id.in_(patient_fhir_ids), Encounter.period_start.isnot(None))
            .group_by("year")
            .order_by("year")
            .all()
        )
        ...
        comparison_data[cohort.name] = {
            "yearly_encounters": {int(row.year): row.count for row in yearly_data if row.year}
        }
