...
async def get_etl_jobs_list(
    ...
    jobs = (
        db.query(ETLJob)
        .filter(ETLJob.status == "completed")
...
async def get_available_diagnoses(
    ...
    for row in results:
        if row.code_text:
            diagnoses.append({"code_text": row.code_text, "count": row.count})
...
async def get_diagnosis_by_code(
    ...
    # Query actual data
    results = (
        query.filter(Condition.onset_datetime >= start_date, Condition.onset_datetime <= end_date)
        .with_entities(
    ...
    for i, (period, count) in enumerate(zip(labels, counts)):
        change = (
            ((count - counts[i - 1]) / counts[i - 1] * 100) if i > 0 and counts[i - 1] > 0 else 0
        )
...
async def get_trends(
    ...
    months: int = Query(
        ...
        ge=1,
        le=120,
        description="Number of months to show (deprecated, use start_year/end_year)",
    ),
    ...
    query = db.query(
        ...
        func.count(Condition.id).label("count"),
    ).filter(Condition.onset_datetime >= start_date, Condition.onset_datetime <= end_date)
...
async def get_top_conditions(
    ...
    query = db.query(Condition.code_text, func.count(Condition.id).label("count")).filter(
    ...
    if start_year and end_year:
        ...
        end_date = datetime(end_year, 12, 31, 23, 59, 59)
        query = query.filter(
            Condition.onset_datetime >= start_date, Condition.onset_datetime <= end_date
        )
    ...
    results = (
        query.group_by(Condition.code_text)
        .order_by(func.count(Condition.id).desc())
        .limit(limit)
        .all()
    )
    ...
    if not results:
        return {"labels": [], "values": [], "percentages": [], "total": 0, "trends": []}
    ...
    if start_year and end_year:
        ...
        if prev_start_year >= 2000:
            ...
            for label in labels:
                ...
                trends.append({"direction": trend, "change": change_percent})
        else:
...
async def get_diagnosis_analysis(
    ...
    periods: int = Query(
        ...
        ge=1,
        le=20,
        description="Number of periods to display (default: 12 for monthly, 8 for quarterly, 5 for yearly, max 20)",
    ),
    job_id: Optional[str] = Query(None, description="Filter by specific ETL job ID"),
    startYear: int = Query(None, ge=1900, le=2100, description="Start year for analysis"),
    endYear: int = Query(None, ge=1900, le=2100, description="End year for analysis"),
    ...
    diagnosis_search_terms = {
        ...
        "hypertension": [
            ...
            "I15",  # ICD-10
            "59621000",
            "38341003",  # SNOMED-CT codes (59621000=Hypertension, 38341003=Essential hypertension)
        ],
        "copd": [
            "copd",
            "chronic obstructive",
    ...
    if search_terms:
        ...
        query = query.filter(Condition.code_text.ilike(f"%{diagnosis}%"))
    ...
    if total_count > 0 and not search_terms:
        ...
        try:
            ...
            result = db.execute(
                sql,
                {"diag_pattern": f"%{diagnosis}%", "start_date": start_date, "end_date": end_date},
            )
            row = result.fetchone()
            if row and row[0]:
                diagnosis_name = row[0]
...
async def get_patient_demographics(
    ...
    patients_with_birthdate = (
        db.query(Patient.birth_date).filter(Patient.birth_date.isnot(None)).all()
    )
    ...
    return {
        ...
        "ageGroups": {"labels": list(age_groups.keys()), "values": list(age_groups.values())},
    }
...
async def get_recent_activities(
    ...
    recent_jobs = db.query(ETLJob).order_by(ETLJob.created_at.desc()).limit(limit).all()
