...
from app.models.fhir_resources import Condition, Encounter, Patient
...
async def kaplan_meier_analysis(
    ...
    try:
        # 構建查詢
        query = db.query(
            Condition.patient_id, Condition.onset_datetime, Patient.gender, Patient.birth_date
            ...
            if record.onset_datetime:
                ...
                if record.birth_date:
                    ...
                    birth_datetime = datetime.combine(
                        record.birth_date, datetime.min.time()
        ...
        if stratify_by and stratify_by in df.columns:
            ...
            for group in groups:
                ...
                if median_survival is not None and not (
                    np.isnan(median_survival) or np.isinf(median_survival)
                ):
                ...
                results_by_group[str(group)] = {
                    "timeline": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.survival_function_.index.tolist()
                    ],
                    "survival_probability": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.survival_function_[str(group)].tolist()
                    ],
                    "confidence_interval_lower": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.confidence_interval_[f"{group}_lower_0.95"].tolist()
                    ],
                    "confidence_interval_upper": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.confidence_interval_[f"{group}_upper_0.95"].tolist()
                    ],
            ...
            if len(groups) == 2:
                ...
                statistical_test = {
                    ...
                    "statistic": (
                        float(test_stat)
                        if not (np.isnan(test_stat) or np.isinf(test_stat))
                        else None
                    ),
                    "p_value": float(p_val) if not (np.isnan(p_val) or np.isinf(p_val)) else None,
                    "significant": (
                        bool(p_val < 0.05) if not (np.isnan(p_val) or np.isinf(p_val)) else False
                    ),
                }
            else:
            ...
            return convert_numpy_types(
                {
                    ...
                    "survival_probability": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.survival_function_["Overall"].tolist()
                    ],
                    "confidence_interval_lower": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.confidence_interval_["Overall_lower_0.95"].tolist()
                    ],
                    "confidence_interval_upper": [
                        float(x) if not (np.isnan(x) or np.isinf(x)) else None
                        for x in kmf.confidence_interval_["Overall_upper_0.95"].tolist()
                    ],
...
async def kaplan_meier_plot(
    ...
    try:
        ...
        if stratify_by and stratify_by in df.columns:
            ...
            for idx, group in enumerate(df[stratify_by].unique()):
                ...
                kmf.plot_survival_function(
                    ax=ax, ci_show=True, color=colors[idx % len(colors)], linewidth=2.5, alpha=0.8
                )
        else:
            # 整體繪圖
            kmf = KaplanMeierFitter()
            ...
            kmf.plot_survival_function(
                ax=ax, ci_show=True, color="#2E86AB", linewidth=2.5, alpha=0.8
            )
        ...
        ax.set_title(
            "Kaplan-Meier Survival Curve", fontsize=16, fontweight="bold", color="#2c3e50", pad=20
        )
    ...
    try:
        ...
        for record in results:
            if record.onset_datetime and record.birth_date:
                duration = min((current_time - record.onset_datetime).days, max_follow_up_days)
                # 將 date 轉換為 datetime 以進行計算
                birth_datetime = datetime.combine(record.birth_date, datetime.min.time()).replace(
                    tzinfo=timezone.utc
                )
        ...
        for covariate in summary.index:
            results_dict[covariate] = {
                ...
                "confidence_interval_lower": float(
                    np.exp(summary.loc[covariate, "coef lower 95%"])
                ),
                "confidence_interval_upper": float(
                    np.exp(summary.loc[covariate, "coef upper 95%"])
                ),
...
async def survival_summary(
    ...
    try:
        ...
        if not results:
            return {"total_patients": 0, "message": "No data found"}
        ...
        return convert_numpy_types(
            {
                ...
                "gender_distribution": {
                    ...
                    "unknown": sum(
                        1 for r in results if not r.gender or r.gender not in ["male", "female"]
                    ),
                },
                "age_groups": {
                    "<18": sum(
                        ...
                        and (
                            current_time
                            - datetime.combine(r.birth_date, datetime.min.time()).replace(
                                tzinfo=timezone.utc
                            )
                        ).days
                        / 365.25
