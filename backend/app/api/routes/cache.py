...
async def get_cache_statistics(current_user: dict = Depends(get_current_user)):
    ...
    if stats and stats.get("status") == "error":
        raise HTTPException(
            status_code=500, detail=f"Error getting cache stats: {stats.get('error')}"
        )
...
async def invalidate_cache_pattern(
    pattern: str, current_user: dict = Depends(require_role(["admin", "engineer"]))
):
    """
    ...
    try:
        ...
        logger.info(
            f"User {current_user.get('sub')} invalidated {deleted_count} cache entries with pattern: {pattern}"
        )
...
async def clear_all_cache_entries(current_user: dict = Depends(require_role(["admin"]))):
    """
...
async def invalidate_diagnosis_cache_endpoint(
    ...
    try:
        ...
        return {"status": "success", "message": "Diagnosis cache invalidated"}
    except Exception as e:
...
async def invalidate_after_etl_endpoint(
    ...
    """
    if not REDIS_AVAILABLE:
        return {"status": "success", "message": "Redis not available, no cache to invalidate"}
...
async def check_cache_health():
    ...
    try:
        ...
        if stats and stats.get("status") != "error":
            ...
            return {"status": "error", "message": f"Redis cache error: {stats.get('error')}"}
    except Exception as e:
        return {"status": "error", "message": f"Error checking cache health: {str(e)}"}
