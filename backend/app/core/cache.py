...
def cache_result(expire_seconds: int = 300, key_prefix: Optional[str] = None) -> Callable:
    """
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"💾 Cached result for {cache_key[:80]} (expires in {expire_seconds}s)"
                    )
...
def get_cache_stats() -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "connected_clients": (
                redis_client.client_list().__len__() if hasattr(redis_client, "client_list") else 0
            ),
        }
    except redis.RedisError as e:
