...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL", "postgresql://fhir_user:changeme@localhost:5432/fhir_analytics"
    )
    ...
    def _validate_jwt_secret(self):
        ...
        # In development, warn if using weak or empty secret
            ...
            if not self.JWT_SECRET:
                ...
                print(
                    "⚠️  WARNING: No JWT_SECRET found. Generated temporary secret for development."
                )
                print("⚠️  For production, set JWT_SECRET environment variable!")
                print(f"⚠️  Generate with: openssl rand -hex 32")
            elif len(self.JWT_SECRET) < 32:
                print(
                    f"⚠️  WARNING: JWT_SECRET is weak ({len(self.JWT_SECRET)} chars). Recommended: 32+ characters"
                )
